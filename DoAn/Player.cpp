...
Player::Player(screen PlayGround) {
...
Player::Player(screen PlayGround, string name, int point, int life, int level) {
    int* background = PlayGround._getinform();
    this->x = (background[2] - background[0]) / 2;
    this->y = background[3] - 3;
    this->name = name;
...
void Player::_move(screen PlayGround, char choose) {
    int* background = PlayGround._getinform();
    switch (choose) {
    case 'W': {
...
bool Player::_checkCollision(int** carPos, int size) {
    int* pos = new int[4];
    pos[0] = this->x;
    pos[1] = this->y;
    pos[2] = pos[0] + 2;
...
void Player::_resetPosition(screen PlayGround) {
    this->_destroy();
    int* background = PlayGround._getinform();
    this->x = (background[2] - background[0]) / 2;
    this->y = background[3] - 3;
...
bool Player::_winCheck() {
    if (this->y <= 2) return 1;
    return 0;
}
bool Player::comp(const Player& a, const Player& b) {
